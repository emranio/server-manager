...
export default async function addSite() {
  ...
  try {
    ...
    const answers = await inquirer.prompt([
      {
        ...
        choices: [
          ...
          {
            name: "Static React Site (SPA with client-side routing)",
            value: "react",
          },
      ...
      },
      {
        type: "input",
        name: "dbPrefix",
        message: "Enter database table prefix (default: wp_):",
        default: "wp_",
        when: (answers) => answers.type === "wp",
        filter: (input) => input.trim(),
        validate: (input) => {
          if (!input.trim()) {
            return "Table prefix cannot be empty";
          }
          if (!/^[a-zA-Z_][a-zA-Z0-9_]*$/.test(input.trim())) {
            return "Invalid prefix. Use only letters, numbers, and underscores. Must start with a letter or underscore.";
          }
          return true;
        },
      },
      {
        ...
        name: "corsOrigin",
        message:
          "Enter CORS origin (leave empty to skip CORS, or * to allow all):",
        default: "",
        when: (answers) => answers.type === "proxy",
        filter: (input) => input.trim(),
      },
    ...
    const dbPrefix = answers.dbPrefix || "wp_";
    ...
    if (parsedDomain.isSubdir) {
      ...
      console.log(
        chalk.green(`✓ Parent domain ${parsedDomain.mainDomain} found\n`),
      );
    }

    // Check if site already exists
    if (siteManager.siteExists(primaryKey)) {
      displayError("Site already exists", {
        message: `Site with domain ${domain} already exists`,
      });
      logger.error("Site creation failed: Site already exists", {
        domain,
        primaryKey,
      });
      return;
    ...
    logger.info("Site initialized with status: initializing", {
      domain,
      primaryKey,
    });
    ...
    try {
      ...
      if (type !== "proxy") {
        ...
        if (type === "site" || type === "react") {
          ...
          if (fs.existsSync(welcomeHtmlPath)) {
            ...
            logger.info("Welcome page copied to site", {
              domain,
              welcomePage: indexHtmlPath,
            });
          }
        }

        createdResources.directory = true;
        logger.info("Site directory created with public folder", {
          domain,
          path: sitePath,
      ...
      if (type === "wp") {
        ...
        wpDetails = await wordpressManager.setupWordPress(
          publicPath,
          domain,
          primaryKey,
          adminPassword,
      ...
      const caddyConfigPath = caddyManager.createConfig(
        primaryKey,
        domain,
        type,
        publicPath,
        { enablePhp, proxyAddress, corsOrigin },
      );
      ...
      if (type === "wp" && wpDetails) {
        ...
        successDetails["DB Table Prefix"] = wpDetails.dbPrefix || "wp_";
      }
      ...
      try {
        ...
        if (createdResources.directory && fs.existsSync(sitePath)) {
          ...
          logger.info("Site directory removed during cleanup", {
            path: sitePath,
          });
        }
        ...
        if (createdResources.caddyConfig) {
          ...
          logger.info("Caddy config removed during cleanup", {
            domain,
            primaryKey,
          });
        }
        ...
        if (createdResources.database) {
          console.log(
            chalk.yellow(
              `   - Database: ${process.env.DB_PREFIX || "site_"}${primaryKey}`,
            ),
          );
        }
        if (createdResources.caddyConfig) {
          console.log(
            chalk.yellow(`   - Caddy config: caddy/${primaryKey}.caddy`),
          );
        }
        console.log(
          chalk.yellow(`   - Site entry in sites.json: ${primaryKey}\n`),
        );
      }
