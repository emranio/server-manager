...
export default async function addSite() {
    ...
    let type, domain, primaryKey, sitePath, parsedDomain, group;
    ...
    try {
        ...
        // For subdirectory sites, check if parent domain exists and inherit group
        if (parsedDomain.isSubdir) {
            const parentKey = generatePrimaryKey(parsedDomain.mainDomain);
            const parentSite = siteManager.getSite(parentKey);
            if (!parentSite) {
            ...
            group = parentSite.group || null;
            console.log(chalk.green(`✓ Parent domain ${parsedDomain.mainDomain} found (group: ${group || 'none'})\n`));
        } else {
            // Prompt for group selection
            const wwwDir = path.join(__dirname, '..', '..', 'www');
            const existingGroups = new Set();

            // Collect groups from sites.json
            const allSites = siteManager.listAllSites();
            for (const key of Object.keys(allSites)) {
                if (allSites[key].group) {
                    existingGroups.add(allSites[key].group);
                }
            }

            // Also collect group directories from www/
            if (fs.existsSync(wwwDir)) {
                const entries = fs.readdirSync(wwwDir, { withFileTypes: true });
                for (const entry of entries) {
                    if (entry.isDirectory() && entry.name !== '.gitkeep') {
                        existingGroups.add(entry.name);
                    }
                }
            }

            const sortedGroups = [...existingGroups].sort();
            const CREATE_NEW = '── Create new group ──';
            const groupChoices = [...sortedGroups.map(g => ({ name: g, value: g })), { name: CREATE_NEW, value: CREATE_NEW }];

            const groupAnswer = await inquirer.prompt([
                {
                    type: 'list',
                    name: 'group',
                    message: 'Select or create a group:',
                    choices: groupChoices
                },
                {
                    type: 'input',
                    name: 'newGroup',
                    message: 'Enter new group name (lowercase, letters, numbers, hyphens):',
                    when: (ans) => ans.group === CREATE_NEW,
                    validate: (input) => {
                        const val = input.trim().toLowerCase();
                        if (!val) return 'Group name is required';
                        if (!/^[a-z0-9]([a-z0-9-]*[a-z0-9])?$/.test(val)) {
                            return 'Invalid group name. Use lowercase letters, numbers, and hyphens. Must not start/end with a hyphen.';
                        }
                        return true;
                    },
            ...
            group = groupAnswer.group === CREATE_NEW ? groupAnswer.newGroup : groupAnswer.group;
        }
        ...
        // Set up directory path (with group subfolder)
        sitePath = group
            ? path.join(__dirname, '..', '..', 'www', group, primaryKey)
            : path.join(__dirname, '..', '..', 'www', primaryKey);
        ...
        try {
            ...
            const caddyConfigPath = caddyManager.createConfig(primaryKey, domain, type, publicPath, { enablePhp, proxyAddress, corsOrigin, group });
            ...
            try {
                ...
                if (createdResources.caddyConfig) {
                    const configName = group ? `${group}__${primaryKey}.caddy` : `${primaryKey}.caddy`;
                    console.log(chalk.yellow(`   - Caddy config: caddy/${configName}`));
                }
