...
class WordPressManager {
     ...
     * @returns {Promise<void>}
     */
    async createConfig(sitePath, dbName, dbUser, dbPassword, dbHost) {
        try {
            const command = `php -d memory_limit=1G $(which wp) config create --path="${sitePath}" --dbname="${dbName}" --dbuser="${dbUser}" --dbpass="${dbPassword}" --dbhost="${dbHost}" --quiet`;
            await executeCommand(command);
        } catch (error) {
     ...
     * @returns {Promise<void>}
     */
    async installWordPress(sitePath, url, title, adminUser, adminPassword, adminEmail) {
        try {
     ...
     */
    async setupWordPress(sitePath, domain, primaryKey, adminPassword) {
        // Check if WP-CLI is installed
        const wpCliInstalled = await this.isWpCliInstalled();
        if (!wpCliInstalled) {
            throw new Error('WP-CLI is not installed. Please install it first: https://wp-cli.org/');
        }
